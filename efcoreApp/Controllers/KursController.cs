...
namespace efcoreApp.Controllers
{
    public class KursController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int?id)
        {
            ...
            var kurs = await _context.Kurslar.Include(k => k.kursKayitlari).ThenInclude(k => k.Ogrenci).Select(k => new KursViewModel { KursId = k.KursId, Baslik = k.Baslik, OgretmenId = k.OgretmenId, kursKayitlari = k.kursKayitlari}).FirstOrDefaultAsync(k => k.KursId == id);
            if (kurs == null)
            {
                return NotFound();
            }
